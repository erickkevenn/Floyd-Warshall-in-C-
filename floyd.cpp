...
vector<vector<int>> criarMatrizAdjacencia(int numVertices) {
    // Inicializar matriz com todos os elementos como infinito (sem arestas)
    vector<vector<int>> matriz(numVertices, vector<int>(numVertices, INT_MAX));
...
void adicionarAresta(vector<vector<int>>& matriz, int vertice1, int vertice2, int peso) {
    matriz[vertice1][vertice2] = peso;
}

// Função para imprimir o menor caminho entre dois vértices
void imprimirMenorCaminho(const vector<vector<int>>& distancias, int origem, int destino) {
...
int main() {
    ...
    vector<vector<int>> matrizAdjacencia = criarMatrizAdjacencia(numVertices);
